...
<div class="bg-gray-50 rounded-lg p-4 sm:p-6 lg:p-8">
    <div class="max-w-7xl w-full mx-auto">
        ...
        <div class="mb-8 text-center sm:text-left">
            <h1 class="text-lg sm:text-xl md:text-2xl font-bold text-gray-900">Edit Produk</h1>
            <p class="mt-1 text-sm text-gray-500">Perbarui detail untuk produk <span class="font-medium text-gray-700">{{ $product->name }}</span>.</p>
        </div>
        ...
        {{-- Form Utama --}}
        <form action="{{ route('products.update', $product->id) }}" method="POST" enctype="multipart/form-data" class="bg-white p-6 sm:p-8 rounded-xl shadow-lg">
            @csrf
            @method('PUT')
            <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="name" id="name" class="block w-full px-4 py-3 border border-gray-300 rounded-lg shadow-sm placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-amber-500 transition" value="{{ old('name', $product->name) }}" required>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="text" name="price" id="price" class="block w-full pl-10 pr-4 py-3 border border-gray-300 rounded-lg shadow-sm placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-amber-500 transition" value="{{ old('price', number_format($product->price, 0, ',', '.')) }}" required oninput="formatRupiah(this)">
                        </div>
                    </div>
                </div>
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <div id="image-upload-box" class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-lg h-full">
                            <div id="image-preview-wrapper" class="{{ $product->image ? '' : 'hidden' }} w-full text-center my-auto">
                                <img id="preview-image" src="{{ $product->image ? asset('storage/' . $product->image) : '#' }}" alt="Pratinjau Gambar" class="mx-auto h-40 rounded-md mb-4 object-contain">
                                <button type="button" id="change-image-btn" class="text-sm font-medium text-amber-600 hover:text-amber-500">Ganti gambar</button>
                            </div>
                            <div id="upload-placeholder" class="{{ $product->image ? 'hidden' : '' }} space-y-1 text-center my-auto">
